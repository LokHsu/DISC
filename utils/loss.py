...
def infonce_loss(tar_user_emb, aux_user_emb, top_sim_idx, tau=0.5):
    ...
    if top_sim_idx != None:
        sim_user_score = torch.matmul(x_norm, y_norm.T)
        sim_user_score[~top_sim_idx] = 0.
        pos_score += torch.sum(sim_user_score, dim=1)
    pos_score = torch.exp(pos_score / tau)

    neg_score = torch.matmul(x_norm, y_norm.T)
    if top_sim_idx != None:
        neg_score[top_sim_idx] = 0.
...
def calc_infonce_loss(user_embs, batch_users, behaviors, top_sim_idx_list=None):
    cl_loss_list = [None] * len(behaviors)
    tar_user_emb = user_embs[-1][batch_users]
    for i in range(len(behaviors)):
        aux_user_emb = user_embs[i][batch_users]

        if top_sim_idx_list != None:
            top_sim_idx = top_sim_idx_list[i]
        else:
            top_sim_idx = None

        cl_loss_list.append(infonce_loss(tar_user_emb, aux_user_emb, top_sim_idx))
